      ...
      tabsetPanel(type = "tabs",
          tabPanel("Home",
                   br(),
                    ...
                    div("EXPONENTIAL - the intersection of the initial density line and the tangent line to the part of the curve where the growth rate is maximal"),
          ...
          fluidRow(column(12, align="center",  
           ...
           sidebarPanel(
             ...
             br(),
             h5("Parameter related to model fitting methods"),
             selectInput("model",
                         label = "Model to fit",
             ...
             numericInput("max.iter",
                          ...
                          step = 100),
             br(),
             h6("Initial parameter values"),
             numericInput("init.lag",
                          label = "Initial guess for the lag",
                          value = 3.5,
                          min = 0, 
                          max = 24,
                          step = 0.1),
             numericInput("init.growth.rate",
                          label = "Initial guess for the max. growth rate",
                          value = 0.1,
                          min = 0, 
                          max = 1,
                          step = 0.01),
             h5("Parameters related to biomass increase method"),
             numericInput("threshold",
                          value = 100,
                          label = "Min. meaningful threshold in biomass increase",
                          min = 0),
             br(),
             h5("Parameters related to the exponential method"),
             selectInput("tangent.method",
                         label = "How to find the exponential phase",
                         choices = c("local.regression", "to.point"),
                         selected = "local.regression"),
             br(),
             h6("If local.regression method is chosen choose the number of points to be included in the regression line"),
...
server <- shinyServer(function(input, output) {
    ...
    model.params = reactive({
      pars = list(model = input$model,
                  ...
                  tangent.method = input$tangent.method,
    ...
    growth.curve.data.with.lag =  reactive({ 
      if (nrow(growth.curve.data()$data) > 0) {
        data.with.lag = Calculate.Lag(data = growth.curve.data()$data, 
                      method = input$method, 
                     pars = model.params()) %>%
        mutate(lag.calculation.method = input$method) 
      } else {
        data.with.lag = growth.curve.data()$data
      }
